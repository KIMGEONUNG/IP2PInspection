...
from ldm.models.diffusion.ddpm_edit import DDPM, disabled_train
...
class LatentRestoration(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        encoder_posterior, _ = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        ...
        cond["c_concat"] = [input_mask * self.encode_first_stage((xc["c_concat"].to(self.device)))[0].mode().detach()]
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
               ...
               mask=None, x0=None, start_T=None, shape=None,**kwargs):
        if shape is None:
        ...
        return self.p_sample_loop(cond,
                                  ...
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised, start_T=start_T,
                                  mask=mask, x0=x0)

    @torch.no_grad()
    def sample_log(self,cond,batch_size,ddim, ddim_steps=None, start_T=None, **kwargs):
