...
def main():
    ...
    # LOAD CONFIG
    config = OmegaConf.load('configs/E003-B.yaml')
    ...
    shortcut = instantiate_from_config(config.shortcut)
    shortcut.eval().cuda()
    ...
    for t_start in [0, 200, 400, 600, 800]:
        ...
        with torch.no_grad(), model.ema_scope():
            for i, path in enumerate(
                ...
                posterior_shortcut = shortcut.encode(input_image)
                cond["c_concat"] = [posterior_shortcut.mode()]
                ...
                # DEFINE TIMESTEP
                z = posterior_shortcut.mode() * model.scale_factor
                if t_start != 0:
