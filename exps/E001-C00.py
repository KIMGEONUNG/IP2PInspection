...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb, skip):
        ...
        """
        return checkpoint(self._forward, (x, emb, skip), self.parameters(),
                          self.use_checkpoint)

    def _forward(self, x, emb, skip):
        if skip:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if timesteps[0] > 800:
            del_encode_idxs = [0,1,2,3]
            del_decode_idxs = [8,9,10,11]
        else:
            del_encode_idxs = []
            del_decode_idxs = []
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            if i in del_encode_idxs:
                skip = True
            # h = module(h, emb, context, skip=skip)
            h = module(h, emb, context, skip)
            hs.append(h)
