...
if __name__ == "__main__":
    ...
    # DEFINE TIMESTEP
    uniform = np.linspace(0, 999, 40).astype('int')
    trunc = np.linspace(0, 999, 40).astype('int')
    trunc = trunc[trunc < 500]
    trunc = np.concatenate([trunc, np.array([500, 750, 999]).astype('int')])
    ...
    with torch.no_grad():
        for i, value in enumerate(tqdm(dataloader)):
            ...
            for key, timesteps in {"uni": uniform, "trunc": trunc}.items():
                ...
                for j, item in enumerate(range(len(x))):
                    ...
                    img_out.save(join(logdir, f"{key}_{i*batch_size+j:04d}.png"))
