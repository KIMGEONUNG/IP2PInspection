    ...
    with torch.no_grad(), model.ema_scope():
        for i, path in enumerate(sorted(glob("DATASET/transform_lf/*.jpg"))):
            ...
            pairs = [
                [
                    "t_o",
                    torch.Tensor([500.]).cuda(),
                ],
                [
                    "t_f",
                    torch.Tensor([200.]).cuda(),
                ],
                [
                    "t_b",
                    torch.Tensor([800.]).cuda(),
                ],
            ]

            rs = []
            for target, t in pairs:
                sigmas = model_wrap.t_to_sigma(t)
                ...
                rs.append(edited_image)

            grid = merge_pil(img_lf, *rs, img_gt)
            grid.save(f"outputs/cmp_nonlinear_timesteps/pair3/{name}.jpg",
                      quality=100,
                      sunbsampling=0)
