...
def main():
    """
    Motivation:
    Now, we fail to an expriment which should succeed, theoreticall.
    ...
    # LOAD CONFIG
    config = OmegaConf.load('configs/generate_shortcut.yaml')
    config.model.params.ckpt_path = "checkpoints/openimage_s/last.ckpt"

    # LOAD MODEL AND DATASET
    model_shortcut = instantiate_from_config(
        config.model_shortcut).eval().cuda()
    ...
    path_dir = "outputs/output_shortcutfb"
    ...
    with torch.no_grad(), model.ema_scope():
        for i, path in enumerate(
            ...
            posterior_gt, intermediate_gt = model.encode_first_stage(input_gt)
            posterior, intermediate = model.encode_first_stage(input_image)
            posterior_shortcut = model_shortcut.encode(input_image)
            ...
            x = torch.clamp((x + 1.0) / 2.0, min=0.0, max=1.0)
            x = 255.0 * rearrange(x, "1 c h w -> h w c")
            edited_from_encoder = Image.fromarray(x.type(torch.uint8).cpu().numpy())


            # SAVE IMAGE
            grid = merge_pil(edited_from_gt, edited_from_encoder, img_gt)
            grid.save(join(path_dir, f"{i:04}.jpg"),
                      quality=100,
                      sunbsampling=0)

            psnr_gt = PSNR(edited_from_gt, img_gt)
            psnr_ee = PSNR(edited_from_encoder, img_gt)

            with open(path_metric, "a") as myfile:
                myfile.write(f"start@{t_start} {i:04}_gt: {psnr_gt}\n")
                myfile.write(f"start@{t_start} {i:04}_ee: {psnr_ee}\n")
