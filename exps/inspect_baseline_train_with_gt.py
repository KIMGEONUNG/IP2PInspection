...
def main():
    ...
    # LOAD CONFIG
    config = OmegaConf.load('configs/generate_baseline_with_gt.yaml')
    config.model.params.ckpt_path = "checkpoints/openimage_s/last.ckpt"

    # LOAD MODEL AND DATASET
    model = instantiate_from_config(config.model)
    model.first_stage_model.init_from_ckpt("checkpoints/fuser.ckpt")
    model.eval().cuda()
    ...
    path_dir = "outputs/output_baseline_train_with_gt"
    ...
    with torch.no_grad(), model.ema_scope():
        for i, path in enumerate(
                sorted(glob("DATASET/transform_lf_test/*.jpg"))):
            ...
            x = model.first_stage_model.decode(z / model.scale_factor, intermediate)
            x = torch.clamp((x + 1.0) / 2.0, min=0.0, max=1.0)
            x = 255.0 * rearrange(x, "1 c h w -> h w c")
            edited_image = Image.fromarray(x.type(torch.uint8).cpu().numpy())

            grid = merge_pil(img_lf, edited_image, img_gt)
            grid.save(f"outputs/output_baseline_train_with_gt/{i:04}.jpg",
                      quality=100,
                      sunbsampling=0)
